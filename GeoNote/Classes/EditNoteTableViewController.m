...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *btnNavSave = [[UIBarButtonItem alloc] initWithTitle:@"Save" style:UIBarButtonItemStylePlain target:self action:@selector(btnSaveClick:)];
    self.navigationController.topViewController.navigationItem.rightBarButtonItem = btnNavSave;
    [btnNavSave release];
    
    // Define the form fields
	NSArray *array = [[NSArray alloc] initWithObjects:@"Title", @"Text", @"Updated", @"Coords", nil];
	...
	self.headerHeight = kDefaultEditNoteHeaderHeight;
	self.footerHeight = kDefaultEditNoteFooterHeight;
}
...
-(IBAction)btnSaveClick:(id)sender {
	...
	NSString *title = self.currentNote.title;
    NSString *text = self.currentNote.text;
    ...
    {
        switch ([key intValue]) {
            case kEditNoteTitleRowIndex:
                title = [self.tempValues objectForKey:key];
                break;
            case kEditNoteTextRowIndex:
    ...
    if([trimmedTitle isEqualToString:@""]) {
		...
		if([trimmedText isEqualToString:@""]) {
            ...
            // Update note entry
            NSManagedObject *itemToUpdate = self.currentNote;
            [itemToUpdate setValue:trimmedTitle forKey:@"title"];
            [itemToUpdate setValue:trimmedText forKey:@"text"];
            [itemToUpdate setValue:[NSDate date] forKey:@"updateDate"];
            ...
            if (![appDelegate.managedObjectContext save:&error]) {
                ...
                // Return to map which will refresh and show updated pin
...
-(IBAction)textFieldDone:(id)sender {
	...
	BOOL isLastField = NO;
	if (row >= kEditNoteNumberOfEditableRows)
	{
		isLastField = YES;
		row = 0;
	}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	return kEditNoteNumberOfEditableRows;
}
	...
	switch (row) {
		case kEditNoteTitleRowIndex:
        {
			if ([[self.tempValues allKeys] containsObject:rowAsNum])
                ...
                textField.text = self.currentNote.title;
			}
		...
		case kEditNoteTextRowIndex:
        {
			if ([[self.tempValues allKeys] containsObject:rowAsNum])
                ...
                textField.text = self.currentNote.text;
			}
        ...
        case kEditNoteUpdatedDateRowIndex:
        {
            ...
            textField.text = [dateFormatter stringFromDate:[self.currentNote updateDate]];
            [textField setEnabled:NO];
            
            break;
        }
        case kEditNoteCoordinateRowIndex:
        {
            ...
            NSString *coordStr = [NSString stringWithFormat:@"(%@, %@)",
                                  [numberFormatter stringFromNumber:[self.currentNote latitude]],
                                  [numberFormatter stringFromNumber:[self.currentNote longitude]]];

            textField.text = coordStr;
            [textField setEnabled:NO];
...
{        
	EditNoteCustomHeader *headerView = nil;
	
	if (headerView == nil) {
		DLog(@"New EditNoteCustomHeader");
		
		NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"EditNoteCustomHeader" owner:nil options:nil];
		
		for(id currentObject in topLevelObjects)
		{
			if([currentObject isKindOfClass:[EditNoteCustomHeader class]])
			{
				headerView = (EditNoteCustomHeader *)currentObject;
				break;
			}
		}
	}
...
-(void)setHeaderTextAndAdjustHeight:(NSString *)text headerView:(EditNoteCustomHeader *)headerView {
	...
	self.headerHeight = headerHeightAfter + kEditNoteHeaderHeightOffset;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
	...
	EditNoteCustomFooter *footerView = nil;
	
	if (footerView == nil) {
		
		DLog(@"New EditNoteCustomFooter");
		
		NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"EditNoteCustomFooter" owner:nil options:nil];
		
		for(id currentObject in topLevelObjects)
		{
			if([currentObject isKindOfClass:[EditNoteCustomFooter class]])
			{
				footerView = (EditNoteCustomFooter *)currentObject;
				break;
			}
		}
	}
